...
<body>
  ...
  <div class="page chart-page">
    ...
    <div class="section">
      ...
      <div class="chart-container" style="border: 2px solid #000; padding: 10px; background: #fff;">
      ...
      <script>
        (function() {
          ...
          const readings = ${JSON.stringify(readings.map(r => ({ x: r.loadT, y: r.avgSettlementMm, phase: r.phase })))};
          ...
          const xMax = Math.ceil(maxLoad / 50) * 50 + 150;
          const yMax = Math.ceil(maxSettlement * 2) / 2 + 1.5;
          
          // For square grids: match the number of grid divisions to chart aspect ratio
          // Chart is roughly 1.6:1 (width:height), so we need proportional steps
          // X step = 50, Y step = 0.5 means ratio of 100:1 in data units
          // To get square cells: xRange/xStep should relate to yRange/yStep by aspect ratio
          const xStep = 50;
          const yStep = 0.50;
          ...
          if (loadingData.length > 0) {
            datasets.push({
              ...
              datalabels: {
                ...
                color: '#000',
                formatter: (value) => value.x.toFixed(2) + ', ' + value.y.toFixed(2)
              }
            });
          }
          if (unloadingData.length > 0) {
            datasets.push({
              ...
              datalabels: {
                ...
                font: { size: 9, weight: 'bold' },
                color: '#000',
                // Skip label for connector point (already labeled in loading)
                formatter: (value) => value.isConnector ? '' : value.x.toFixed(2) + ', ' + value.y.toFixed(2)
              }
            });
          }
          
          // Calculate aspect ratio for square grid cells
          const xDivisions = xMax / xStep;
          const yDivisions = yMax / yStep;
          const aspectRatio = xDivisions / yDivisions;
